...
import com.choa.musinsai.core.provider.crawler.shop.Shop
import com.choa.musinsai.core.provider.crawler.shop.ShopCrawler
import com.choa.musinsai.core.provider.crawler.shop.ShopDetail
import com.choa.musinsai.core.provider.crawler.shop.ShopDetailRequest
import com.choa.musinsai.core.provider.crawler.shop.ShopDetailResponse
import com.choa.musinsai.core.provider.crawler.shop.ShopFloorInfo
import com.choa.musinsai.core.provider.crawler.shop.ShopSearchRequest
import com.choa.musinsai.core.provider.crawler.shop.ShopSearchResponse
...
@Component
class MusinsaOfflineShopCrawler(
    ...
    fun getShops(): ShopSearchResponse = getShops(ShopSearchRequest())
    fun getShops(shopType: String?): ShopSearchResponse = getShops(ShopSearchRequest(shopType = shopType))
    fun getShops(shopType: String?, region: String?, language: String): ShopSearchResponse =
        getShops(ShopSearchRequest(shopType = shopType, region = region, language = language))

    fun getShopDetail(shopNo: Long, language: String = "ko"): ShopDetailResponse =
        getShopDetail(ShopDetailRequest(shopNo = shopNo, language = language))

    override fun getShops(request: ShopSearchRequest): ShopSearchResponse {
        return try {
            ...
            ShopSearchResponse(total = res.data.total, shops = shops)
        } catch (e: Exception) {
            logger.error(e) { "무신사 오프라인 매장 조회 중 오류 발생" }
            ShopSearchResponse(total = 0, shops = emptyList())
        }
    }

    override fun getShopDetail(request: ShopDetailRequest): ShopDetailResponse {
        return try {
            ...
            val mapped = ShopDetail(
                ...
                paymentServices = d.paymentServices,
            ...
            ShopDetailResponse(detail = mapped)
        } catch (e: Exception) {
            logger.error(e) { "무신사 오프라인 매장 상세 조회 중 오류 발생" }
            ShopDetailResponse(detail = null)
        }
    }
